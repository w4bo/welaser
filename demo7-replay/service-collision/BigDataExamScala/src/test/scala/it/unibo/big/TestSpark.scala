...
class TestSpark {
  ...
  @Test
  def `Test Geospark -- read geojson`: Unit = {
    ...
    val municipalityRDD = GeoJsonReader.readToGeometryRDD(sparkSession.sparkContext, inputLocation, allowTopologyInvalidGeometries, skipSyntaxInvalidGeometries)
    val municipalityDF = Adapter.toDf(municipalityRDD, sparkSession)
    print("spatial df schema")
    municipalityDF.printSchema()
  }

  @Test
  def `Test kafka - create topic`: Unit = {
